...
def main(req: func.HttpRequest) -> func.HttpResponse:
    logging.info(
        'Python HTTP trigger function processed a request to do a database restore.')
    restore_db = req.params.get('restore_db')
    container_name = req.params.get('container_name')
    blob = req.params.get('blob')
    download_file_path = download_backup_container(
        container_name=container_name, blob=blob)
